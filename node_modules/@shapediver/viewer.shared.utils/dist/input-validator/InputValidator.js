...
let InputValidator = class InputValidator {
    ...
    validateAndError(topic, scope, value, type, defined = true, enumValues = []) {
        ...
        if (res)
            return;
        this._logger.error(topic, new SDError_1.SDError(`${scope}: Input could not be validated. ${value} is not of type ${type}.${defined === false ? ' (Can also be undefined)' : ''}`), `${scope}: Input could not be validated. ${value} is not of type ${type}.${defined === false ? ' (Can also be undefined)' : ''}`, true);
    }
